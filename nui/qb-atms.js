...
function populateCurrentCards(data) {
    ...
    $.each(data.cards, function(index, card) {
        ...
        if(card.cardActive !== undefined && card.cardActive !== null && card.cardActive === true) {
            ...
            $('#cardsDisplay').append('<div class="card mb-3 mx-auto" style="width:100%; background:#212529; border-radius:15px; box-shadow: 0 3px 10px rgb(0 0 0 / 0.2); color: #dcdcdc"><div class="row no-gutters"><div class="col-2 my-auto text-center"><img src="images/' + image + '" class="img-fluid" width="100" alt=""></div><div class="col-8"><div class="card-body p-0"><h5 class="card-title pb-0 pt-1 pl-2 mb-0" style="color: #fff" id="cardOwner-' + index + '">' + card.name + '</h5><p class="card-text pl-2"><small class="text-muted"><span style="color: #fff" id="cardType-' + index + '">' + cardStatus + '' + card.cardType + '</span><br><span style="color: #a8a8a8; font-size: 12px;" id="cardNumber-' + index + '">' + cardNumber + '</span></small></p></div></div><div class="col-2 my-auto pr-1"><button class="btn btn-dark btn-sm btn-block" style="font-size: 12px;" id="cardBtn-' + index + '" data-action="useCard">Kullan</button></div></div></div>');
        } else {
            cardStatus = '';
